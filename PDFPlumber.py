...
def extract_data(pdf_path, format_settings):
    df = extract_data_d(pdf_path, format_settings["crop_settings"], format_settings["extract_settings"],
                    ...
                    format_settings["agent_pattern"],format_settings["emmission_pattern"])
    return df

def extract_data_d(pdf_path, crop_settings, extract_settings, columns, data_row_top=1, data_row_bot=1, n_pages=0, 
                   mandatory_columns=[], emmission_format="", agent_pat="",emmission_pat=""):
    with pdfplumber.open(pdf_path) as pdf:
        ...
        for page in pdf.pages[:n_pages]: 
            ...
            try: 
                ...
                # print(f"Warning: page could not be processed. Error: {e}")
                continue

            if emmission_pat and len(pdf.pages) > 0 :
                match = pdf.pages[0].search(emmission_pat, regex=True, case=False)
                if match:
                    suffix = match[0]["groups"][0]
                    dat_sep = suffix[2]
                    df_page["Emmission"] = df_page["Emmission"].where(
                        ~df_page["Emmission"].astype(str).str.contains(r"\d"),
                        df_page["Emmission"].astype(str) + dat_sep + suffix
                    )
...
def extract_data_batch(batch,output_file="output_combined.csv"):
    ...
    df.to_csv("output/"+output_file, index=False, sep=";", decimal=",", encoding="utf-16")
    for company,path,settings in batch:
        ...
        try:
            ...
            print(f"========/!\\ file could not be processed. Error: {e} /!\\========")
            error_lst[path]=e
            continue
        df_temp["Compagnie"] = company
        #df = pd.concat([df, df_temp], ignore_index=True)
        df_temp.to_csv("output/"+output_file, mode='a', header=False, index=False, sep=";", decimal=",", encoding="utf-16")
        print(f"Processed {len(df_temp)} transactions succesfully")
        if error_lst:
            print(error_lst)
    print(f"Processed a total of {pd.read_csv('output/'+output_file,sep=";",encoding="utf-16").shape[0]} transactions.")